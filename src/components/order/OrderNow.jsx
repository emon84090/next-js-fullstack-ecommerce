...
export default function OrderNowPage() {
  ...
  const handleLivePayment = async (paymentMethodName) => {
    if (!cartId || !selectedAddressId) {
      toast.error("Please ensure your cart and shipping address are selected.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch("/api/payment/initiate", {
        ...
        body: JSON.stringify({
          ...
          customerPhone: selectedAddress.phoneNumber || "01700000000",
          customerAddress: selectedAddress.street || "N/A",
          customerCity: selectedAddress.city || "N/A",
          customerCountry: selectedAddress.country || "Bangladesh",
        }),
      });
      ...
      if (response.ok && result.redirectUrl) {
        ...
        throw new Error(
          result.error || `Failed to initiate ${paymentMethodName} payment`
        );
      }
    } catch (error) {
      console.error(`Error initiating ${paymentMethodName} payment:`, error);
